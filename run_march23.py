"""Retroactive March 23 session runner — fetches 1-min intraday, runs open/midday/close."""
...
TICKERS = [
    # Currently held (from state files as of March 20 close)
    "C","DBC","GLD","GLW","GOOGL","IEMG","IWM","MSFT","NEM","SHY","SPY","UUP","V","WFC",
    # Likely new signals
    "AME","AXP","CCL","CIEN","COO","EA","FCX","FDX","FE","FIX","GEV","GOOG","GLW","HWM",
    "HOLX","IFF","JNJ","KR","L","LII","LRCX","LUV","MCHP","MU","NRG","NTAP",
    "PG","PH","PPG","PRU","PWR","QQQ","RTX","SNDK","STX","TER","TFC","TLT",
    "TSLA","UAL","UHS","WBD","WDC","XLB","XLE","XLF","XLI","XLK","XLP","XLU","XLV","XLY","VXX",
]
TICKERS = sorted(set(TICKERS))  # deduplicate

PRICE_JSON = 'state/intraday_prices_2026-03-23.json'
DATE_STR   = '2026-03-23'
...
if not os.path.exists(PRICE_JSON):
    ...
    sys.stdout.flush()
    raw = yf.download(TICKERS, start=DATE_STR, end='2026-03-24',
    ...
    for sid, trades in session_trades[sess].items():
        if trades:
            ...
            for t in trades:
                ...
                print(f"    {sid}  {t['action']:<4} {t['ticker']:<6}  ${value:,.0f}{note}")
if not any_trade:
    print("\n  No trades executed on March 23.")
