"""Retroactive March 11 session runner — fetches intraday prices then runs open/midday/close."""
...
# ─── Ticker universe (union of Mar-10 set + current positions) ───────────────
TICKERS = [
    "AAPL","AME","AXP","BBAI","C","CCL","CIEN","COO","DBC","EA","EFA",
    "FCX","FDX","FE","FIX","GEV","GLD","GLW","GOOG","GOOGL","HOLX","HWM",
    "IEMG","IFF","IVZ","IWM","JNJ","KR","L","LII","LRCX","LUV","MCHP",
    "MSFT","MU","NEM","NRG","NTAP","PG","PH","PPG","PRU","PWR","QQQ",
    "RTX","SHY","SNDK","SPY","STX","TER","TFC","TLT","TSLA","UAL","UHS",
    "UUP","V","VXX","WBD","WDC","WFC",
]

PRICE_JSON = 'state/intraday_prices_2026-03-11.json'

def price_at(df: pd.DataFrame, hour: int, minute: int) -> dict:
    """Return last 1-min close at or before HH:MM as {ticker: price}."""
...
if not os.path.exists(PRICE_JSON):
    print("Fetching 1-min intraday data for 2026-03-11 ...")
    sys.stdout.flush()
...
print(f"\nRunning 3 sessions for {len(STRATS)} strategies on 2026-03-11")
sys.stdout.flush()
...
for session_name, session_prices in SESSIONS:
    ...
    for sid in STRATS:
        ...
        # Fallback: use entry price for any held ticker not in the price feed
...
with open('state/paper_log.csv', 'a', newline='') as f:
    w = csv.writer(f)
    for sid in STRATS:
        w.writerow(['2026-03-11', sid, round(results[sid], 2)])
