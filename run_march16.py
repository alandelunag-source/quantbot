"""Retroactive March 16 session runner — fetches 1-min intraday, runs open/midday/close."""
...
TICKERS = [
    # Currently held (from state files as of March 13 close)
    "C","CCL","CIEN","DBC","EA","FIX","GEV","GLD","GOOG","GOOGL",
    "HOLX","HWM","IEMG","IWM","JNJ","LRCX","MU","NEM","PWR",
    "SHY","SNDK","SPY","STX","TER","UUP","V","WBD","WDC",
    # Likely new signals
    "AME","AXP","COO","FCX","FDX","FE","GLW","IFF","KR","L","LII","LUV",
    "MCHP","MSFT","NRG","NTAP","PG","PH","PPG","PRU","QQQ",
    "RTX","TFC","TLT","TSLA","UAL","UHS","WFC","XLB","XLE","XLF","XLI","XLK",
    "XLP","XLU","XLV","XLY","VXX",
]

PRICE_JSON = 'state/intraday_prices_2026-03-16.json'
DATE_STR   = '2026-03-16'
...
if not os.path.exists(PRICE_JSON):
    ...
    sys.stdout.flush()
    raw = yf.download(TICKERS, start=DATE_STR, end='2026-03-17',
...
for session_name, session_prices in SESSIONS:
    ...
    for sid in STRATS:
        ...
        try:
            ...
            recent  = [t for t in trades if t.get('date') == DATE_STR]
            trade_str = (f"  {len(recent)} trade(s): " +
                         ", ".join(f"{t['action']} {t['ticker']}" for t in recent[:3])
                         ) if recent else ""
