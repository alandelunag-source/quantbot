"""March 12 midday session — uses already-fetched intraday prices at 12:30 ET."""
import json, sys, os
...
print(f"Running MIDDAY session for {len(STRATS)} strategies on 2026-03-12")
print(f"(12:30 ET prices: {len(intraday['midday'])} tickers)")
sys.stdout.flush()

session_prices = intraday['midday']
...
for sid in STRATS:
    ...
    try:
        summary = ft.update(prices, session='midday')
...
print("\nMidday session complete. Run close at 15:45 ET.")
