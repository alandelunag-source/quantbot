"""Retroactive March 17 session runner — fetches 1-min intraday, runs open/midday/close."""
...
TICKERS = [
    # Currently held (from state files as of March 16 close)
    "C","CCL","CIEN","DBC","EA","FE","FIX","GLD","GOOG","GOOGL",
    "HOLX","HWM","IEMG","IWM","JNJ","LRCX","MSFT","MU",
    "SHY","SNDK","SPY","STX","TER","UUP","V","WBD","WDC",
    # Likely new signals
    "AME","AXP","COO","FCX","FDX","GEV","GLW","IFF","KR","L","LII","LUV",
    "MCHP","NEM","NRG","NTAP","PG","PH","PPG","PRU","PWR","QQQ",
    "RTX","TFC","TLT","TSLA","UAL","UHS","WFC","XLB","XLE","XLF","XLI","XLK",
    "XLP","XLU","XLV","XLY","VXX",
]

PRICE_JSON = 'state/intraday_prices_2026-03-17.json'
DATE_STR   = '2026-03-17'
...
if not os.path.exists(PRICE_JSON):
    ...
    sys.stdout.flush()
    raw = yf.download(TICKERS, start=DATE_STR, end='2026-03-18',
...
for session_name, session_prices in SESSIONS:
    ...
    for sid in STRATS:
        ...
        ft  = ForwardTest(cls())
        # Snapshot trades before update
        trades_before = set(
            (t['date'], t['action'], t['ticker'], t.get('shares', 0))
        ...
        try:
            ...
            # Find new trades from this session
            new_trades = [
                t for t in summary.get('trades', [])
                if (t['date'], t['action'], t['ticker'], t.get('shares', 0)) not in trades_before
            ]
...
for sid in STRATS:
    cls = ALL_STRATEGIES[sid]
    ft  = ForwardTest(cls())
    pv  = ft._state.get('portfolio_value', 0)
    results[sid] = pv
    ...
    for sid, trades in session_trades[sess].items():
        if trades:
            ...
            for t in trades:
                ...
                price  = t.get('price', 0)
                value  = shares * price
                print(f"    {sid}  {action:<4} {ticker:<6}  {shares:>8.1f} sh @ ${price:.2f}  = ${value:,.0f}")
if not any_trade:
    print("\n  No trades executed on March 17.")
