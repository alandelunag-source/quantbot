"""Retroactive March 25 session runner — fetches 1-min intraday, runs open/midday/close."""
...
TICKERS = sorted(set([
    # Currently held (from state files as of March 24 close)
...
PRICE_JSON = 'state/intraday_prices_2026-03-25.json'
DATE_STR   = '2026-03-25'
...
if not os.path.exists(PRICE_JSON):
    ...
    sys.stdout.flush()
    raw = yf.download(TICKERS, start=DATE_STR, end='2026-03-26',
...
# ─── Actual trades from state files (correct approach) ───────────────────────
print(f"\n{'='*60}")
...
if not any_trade:
    print("\n  No trades executed on March 25.")
